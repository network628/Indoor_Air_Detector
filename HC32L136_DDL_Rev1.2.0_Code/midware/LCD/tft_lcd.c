...
//	LCD_LED(0);
	...
	LCD_RD(1); 
	dat = 0x3f00 & DATAIN;
	    DATAOUT((RGB_Code>>8)|dat);
			LCD_WR(0);
			LCD_WR(1);
//			opt_delay(5);
		  DATAOUT(RGB_Code|dat);
			LCD_WR(0);
			LCD_WR(1);	
	LCD_CS(1);
...
{
	  stc_gpio_config_t LCDIoCfg;	
	  stc_gpio_config_t LCDGpioCfg;	
	  
	  DDL_ZERO_STRUCT(LCDGpioCfg);
    ///< 打开GPIO外设时钟门控
    Sysctrl_SetPeripheralGate(SysctrlPeripheralGpio, TRUE);
    ///< 端口方向配置->输出
    LCDGpioCfg.enDir = GpioDirOut;
    ///< 端口驱动能力配置->高驱动能力
    LCDGpioCfg.enDrv = GpioDrvH;
    ///< 端口上下拉配置->无上下拉
    LCDGpioCfg.enPuPd = GpioNoPuPd;  //GpioPu ;  //GpioNoPuPd;
    ///< 端口开漏输出配置->开漏输出关闭
    LCDGpioCfg.enOD = GpioOdDisable;
    ///< 端口输入/输出值寄存器总线控制模式配置->AHB 
	  LCDGpioCfg.enCtrlMode = GpioAHB;  //GpioFastIO ;   //GpioAHB;
//	
    Gpio_Init(GpioPortD, GpioPin7, &LCDGpioCfg);  //LCD背光
    Gpio_ClrIO(GpioPortD, GpioPin7);
	  Gpio_Init(GpioPortA, GpioPin15,&LCDGpioCfg);  //LCD_CS 
		Gpio_Init(GpioPortC, GpioPin10,&LCDGpioCfg);  //LCD_RD
		Gpio_Init(GpioPortC, GpioPin11,&LCDGpioCfg);  //LCD_WR
		Gpio_Init(GpioPortC, GpioPin12,&LCDGpioCfg);  //LCD_RS
	
	  DDL_ZERO_STRUCT(LCDIoCfg);
    ///< 端口方向配置->输出
    LCDIoCfg.enDir = GpioDirOut;
    ///< 端口驱动能力配置->高驱动能力
    LCDIoCfg.enDrv = GpioDrvH;
    ///< 端口上下拉配置->无上下拉
    LCDIoCfg.enPuPd = GpioNoPuPd;  //GpioPu ;  //GpioNoPuPd;
    ///< 端口开漏输出配置->开漏输出关闭
    LCDIoCfg.enOD = GpioOdDisable;
    ///< 端口输入/输出值寄存器总线控制模式配置->AHB 
	  LCDIoCfg.enCtrlMode = GpioFastIO;  //GpioFastIO ;   //GpioAHB;
		
	  Gpio_Init(GpioPortB, GpioPin0,&LCDIoCfg);  //LCD_DB0 
    Gpio_Init(GpioPortB, GpioPin1,&LCDIoCfg);  //LCD_DB1
    Gpio_Init(GpioPortB, GpioPin2,&LCDIoCfg);  //LCD_DB2 
	  Gpio_Init(GpioPortB, GpioPin3,&LCDIoCfg);  //LCD_DB3
		Gpio_Init(GpioPortB, GpioPin4,&LCDIoCfg);  //LCD_DB4
		Gpio_Init(GpioPortB, GpioPin5,&LCDIoCfg);  //LCD_DB5
		Gpio_Init(GpioPortB, GpioPin6,&LCDIoCfg);  //LCD_DB6
		Gpio_Init(GpioPortB, GpioPin7,&LCDIoCfg);  //LCD_DB7
